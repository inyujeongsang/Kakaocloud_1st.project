...
import kakao.project.arrange.SeatColor;
...
public class ArrangeFrame extends JFrame implements SeatColor{
	...
	private JPanel panel_seat; // panel_seat를 멤버 변수로 선언
...
//				sdtm.loadStudentData();
	...
	public void changePresentSeatNumber() {
		...
		try {
			...
			for(int i=0; i<buttonsArray.length; i++) {
				...
				students.get(i).setCurrentSeatNumber(i+1);
				output.print(students.get(i).getsID() +  " ");
				output.print(students.get(i).getsName() +  " ");
				output.print(students.get(i).getsSex() +  " ");
				output.print((i+1) +  " ");
				output.print(students.get(i).getReservedSeatNumber() +  " ");
				output.print(students.get(i).isReservedSeatStatus() +  " ");
				output.print(students.get(i).getReservedSeatReason() +  " ");
				output.println();
			}
			studentManager.setStudentArray((ArrayList<Student>) students);
			output.close();
			System.out.println("저장 완료 되야한다 꼭!!!!");
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	private void exchangeStudentInformation(String sourceSeatNumber, String targetSeatNumber) {
		...
		//좌석번호 교환하는 로직
		try(Scanner scanner2 = new Scanner(new File("Data.txt"))){
			while (scanner2.hasNextLine()) {
				String line = scanner2.nextLine();
				String[] data = line.split(" ");
				// 학생 객체 생성 및 데이터 설정
				Student student = new Student();
				student.setsID(Integer.parseInt(data[0]));
				student.setsName(data[1]);
				student.setsSex(data[2]);

				if(data[1].equals(sourceSeatNumber)){
					student.setCurrentSeatNumber(realTargetSeatNumber);
				} else if(data[1].equals(targetSeatNumber)){
					student.setCurrentSeatNumber(realSourceSeatNumber);
				} else {
					student.setCurrentSeatNumber(Integer.parseInt(data[3]));
				}
				student.setReservedSeatNumber(Integer.parseInt(data[4]));
				student.setReservedSeatStatus(Boolean.parseBoolean(data[5]));
				student.setReservedSeatReason(data[6]);
				//student를 하나씩 list배열에 담아준다.
		...
		try (PrintWriter writer = new PrintWriter(new File("Data.txt"))) {
			for (Student student : students) {
				// 파일에 학생 정보 작성
				writer.println(student.getsID() + " " + student.getsName() + " " + student.getsSex() + " " + student.getCurrentSeatNumber()+" "+student.getReservedSeatNumber()+" "+student.getReservedSeatStatus()+" "+student.getReservedSeatReason());
			}
		} catch (IOException e) {
			e.printStackTrace();
		}

		//드래그드랍 반영하여 그림 다시 그리기
		repaintSeatsBasedOnFileData();
	}
//
	/* *********************************
	 *    Drag & Drop END *************
	...
	public void repaintSeatsBasedOnFileData() {
		List<Student> students = new ArrayList<>();
		...
		// 화면 업데이트
		for (Student student : students) {
			int seatIndex = student.getCurrentSeatNumber() - 1;
			if (seatIndex >= 0 && seatIndex < buttonsArray.length) {
				buttonsArray[seatIndex].setText(student.getsName());
				Color color = student.getsSex().equals("남") ? new Color(255, 219, 204) : new Color(212, 240, 240);
				buttonsArray[seatIndex].setBackground(color);
			}
		}
		// GUI 컴포넌트 업데이트
		panel_seat.revalidate();
		panel_seat.repaint();
	...
	public void fetchStudents(ArrangeManager arrm){
		for(int i=1; i < arrm.getSeats().size(); i++) {
			if(arrm.getSeats().get(i).getStudent() != null) {
				...
				if(arrm.getSeats().get(i).getStudent().getsSex().equals("남")) {
					setMenSeatColor(i);
				}else if(arrm.getSeats().get(i).getStudent().getsSex().equals("여")) {
					setWomenSeatColor(i);
				}
			}
		}
	...
	public void setMenSeatColor(int i) {
		Color c = new Color(255, 219, 204) ;

		buttonsArray[i-1].setBackground(c);
	}

	@Override
	public void setWomenSeatColor(int i) {
		Color c = new Color(212, 240, 240) ;
	...
	public void setReaderSeatColor(int i) {
		buttonsArray[i-1].setBackground(Color.YELLOW);
	}
}
